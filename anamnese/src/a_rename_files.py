...
def get_person_code(config_general):
    ...
    while pessoa not in list(reverse_person_codes):
        pessoa = input(
            f"Qual pessoa está gravando {list(reverse_person_codes)}: "
        )
    ...
    for index, row in files_df.iterrows():
        ...
        if file_id[-1].isalpha():
            ...
            if original_file_id in files_df['file_id'].values:
                files_df.loc[files_df['file_id'] == original_file_id, 'is_selected'] = False
...
def rename_files_to_standard(files_dict, config_anamnese, config_general):
    ...
    for camera, files in tqdm(files_dict.items()):
        ...
        for file_id, file_path, is_selected in df_files.itertuples(index=False):
            ...
            matching_id = df_sentences[df_sentences["ID"] == file_id]["ID Sentenca"].squeeze()

            if matching_id:
                root_folder_str = config_anamnese["root_folder"] if is_selected else config_anamnese["root_folder_errors"]
                ...
                add_logs(file_path, new_file_path)
