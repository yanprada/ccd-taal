...
def get_anamnese_sentence_ids_dataframe(config_anamnese):
...
def get_select_files_dataframe(files):
    ...
    for index, row in files_df.iterrows():
        ...
        if file_id[-1].isalpha():
            ...
            for older_file_id in older_file_ids:
                files_df.loc[files_df["file_id"] == older_file_id, "is_selected"] = (
                    False
                )
...
def rename_files_to_standard(files_dict, config_anamnese, config_general):
    ...
    df_sentences = get_anamnese_sentence_ids_dataframe(config_anamnese)
